...
public class ShopCartService {
    ...
    public AppResponse addShopCart(ShopCartInfo shopCartInfo){
        ...
        if(goods.getInventory() < shopCartInfo.getGoodsCount()){
            return AppResponse.bizError("当前商品库存不足，无法添加到购物车！");
        }
        //这里判断添加到购物车的商品是否已下架
        if(goods.getIsDeleted() == 1){
            return AppResponse.bizError("无法添加到购物车，当前商品已被下架！");
        }
        //获取总价
        double a = Double.valueOf(shopCartInfo.getGoodsCount());
        double b = Double.valueOf(goods.getNowPrice());
